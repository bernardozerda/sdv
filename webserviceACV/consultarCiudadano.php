    ...
    if (empty($arrErrores)) {
        ...
        if ($usuariovalido) {
            ...
            $sql = "SELECT
                    ...
                    FROM ((((((((((sdht_subsidios.t_frm_formulario t_frm_formulario
                    INNER JOIN sdht_subsidios.t_frm_modalidad t_frm_modalidad
                    ...
                    INNER JOIN
                    sdht_subsidios.t_ciu_tipo_documento t_ciu_tipo_documento
                    ON (t_ciu_ciudadano.seqTipoDocumento =
                    t_ciu_tipo_documento.seqTipoDocumento))
                    INNER JOIN sdht_subsidios.t_ciu_etnia t_ciu_etnia
                    ON (t_ciu_ciudadano.seqEtnia = t_ciu_etnia.seqEtnia))
                    INNER JOIN sdht_subsidios.t_ciu_estado_civil t_ciu_estado_civil
                    ON (t_ciu_ciudadano.seqEstadoCivil =
                    t_ciu_estado_civil.seqEstadoCivil))
                    INNER JOIN sdht_subsidios.t_ciu_sexo t_ciu_sexo
                    ON (t_ciu_ciudadano.seqSexo = t_ciu_sexo.seqSexo))
                    INNER JOIN sdht_subsidios.t_frm_tipovictima t_frm_tipovictima
                    ON (t_ciu_ciudadano.seqTipoVictima =
                    t_frm_tipovictima.seqTipoVictima))
                    INNER JOIN sdht_subsidios.t_ciu_parentesco t_ciu_parentesco
                    ON (t_frm_hogar.seqParentesco = t_ciu_parentesco.seqParentesco))
                    INNER JOIN sdht_subsidios.t_frm_estado_proceso t_frm_estado_proceso
                    ON (t_frm_formulario.seqEstadoProceso =
                    t_frm_estado_proceso.seqEstadoProceso))
                    INNER JOIN sdht_subsidios.t_frm_etapa t_frm_etapa
