...
//        echo "<br>***" . $registros . " fchEscrituraIdentificacion -> " . strtotime($fchEscrituraIdentificacion) . " fchMatricula-> " . strtotime($fchMatricula);
        if (strtotime($fchEscrituraIdentificacion) == "" || strtotime($fchMatricula) == "" || strtotime($fchEscrituraTitulo) == "") {
            $casilla .= (strtotime($fchEscrituraIdentificacion) == '') ? "12" : '';
            $casilla .= (strtotime($fchMatricula) == '') ? "26" : '';
            $casilla .= (strtotime($fchEscrituraTitulo) == '') ? "11" : '';
            $band = 1;
        }
...
function generarLinks($arreglo, $tipo) {
    ...
    $tabla .= "</tr>";
    $int = 1;
    foreach ($arreglo['numdocumento'] as $key1 => $value1) {
        $tabla .= "<tr>";
        $tabla .= "<td>" . $arreglo['numdocumento'] . "</td>";
        if ($tipo == 1) {
            $tabla .= "<td>29</td>";
