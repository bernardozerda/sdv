	...
	<div id="contenidos" class="container">
		...
		<div class="well">
			...
			if (isset($_FILES["archivo"]) && is_uploaded_file($_FILES['archivo']['tmp_name'])) {
				...
				/*$sino = '<SELECT><OPTION VALUE=\"SI\">SI</OPTION><OPTION VALUE=\"NO\">NO</OPTION></SELECT>';
				$sinoaplica = '<SELECT><OPTION VALUE=\"SI\">SI</OPTION><OPTION VALUE=\"NO APLICA\">NO APLICA</OPTION></SELECT>';*/
				$sino = "";
				$sinoaplica = "";
				...
				$consulta = "SELECT T_DES_ESCRITURACION.seqFormulario AS 'HOGAR', 
								...
								'' AS 'FECHA_FOLIO', 
								'$sino' AS 'RESOLUCION_DE_VINCULACION_COINCIDENTE', 
								'$sino' AS 'BENEFICIARIOS_DEL_SDV_COINCIDENTES', 
								'$sino' AS 'NOMBRE_Y_CEDULA_PROPIETARIOS_CTL_INCIDENTES', 
								'$sino' AS 'CONSTITUCION_PATRIMONIO_FAMILIA', 
								'$sino' AS 'INDAGACION_AFECTACION_A_VIVIENDA_FAMILIAR', 
								'$sino' AS 'RESTRICCIONES', 
								'$sino' AS 'ESTADO_CIVIL_COINCIDENTE', 
								'$sino' AS 'CARTA_DE_VINCULACION_RESOLUCION_PROTOCOLIZADA', 
								'' AS 'No_DE_ANOTACION_CTL_COMPRAVENTA', 
								'$sino' AS 'SE_CANCELA_HIPOTECA_MAYOR_EXTENSION', 
								'$sinoaplica' AS 'PATRIMONIO_DE_FAMILIA_REGISTRADO', 
								'$sino' AS 'PROHIBICION_DE_TRANSFERENCIA_Y_DERECHO_DE_PREFERENCIA_REGISTRADOS', 
								'$sinoaplica' AS 'IMPRESION_DE_CONSULTA_FONVIVIENDA', 
								'' AS 'ELABORO', 
								'' AS 'APROBO', 
								'$sino' AS 'SE_VIABILIZA_JURIDICAMENTE', 
								'' AS 'OBSERVACION'
				...
				if ($error == '') {
					$fondoGris = "bgcolor= '#A6A6A6'";
					$fondoRojo = "bgcolor= '#FF0000'";
					print_r ("<table border='1' align='center' cellpadding=0 cellspacing=0>");
					print_r ("<tr align='center'><td>HOGAR</td><td>CC POSTULANTE PRINCIPAL</td><td>TIPO DE DOCUMENTO</td><td>NOMBRE POSTULANTE PRINCIPAL</td><td>PROYECTO</td><td>PROPIETARIO</td><td>seqUnidadProyecto</td><td>txtnombreunidad</td><td>DIRECCION INMUEBLE</td><td>CERTIFICADO DE EXISTENCIA Y HABITABILIDAD</td><td>ESCRITURA REGISTRADA</td><td>FECHA ESCRITURA</td><td>NOTARIA</td><td>CIUDAD NOTARIA</td><td>FOLIO DE MATRICULA</td><td>VALOR INMUEBLE</td><td>NUMERO DEL ACTO</td><td>FECHA DEL ACTO</td><td $fondoGris>No. ESCRITURA</td><td $fondoGris>FECHA ESCRITURA (D/M/A)</td><td $fondoGris>NOTARIA</td><td $fondoGris>CIUDAD NOTARIA</td><td $fondoGris>FOLIO DE MATRICULA</td><td $fondoGris>ZONA OFICINA REGISTRO</td><td $fondoGris>CIUDAD OFICINA REGISTRO</td><td $fondoGris>FECHA FOLIO (D/M/A)</td><td $fondoRojo>RESOLUCION DE VINCULACION COINCIDENTE</td><td $fondoRojo>BENEFICIARIOS DEL SDV COINCIDENTES</td><td $fondoRojo>NOMBRE Y CEDULA DE LOS PROPIETARIOS EN EL CTL INCIDENTES</td><td $fondoGris>CONSTITUCION PATRIMONIO FAMILIA</td><td $fondoGris>INDAGACION AFECTACION A VIVIENDA FAMILIAR</td><td $fondoGris>RESTRICCIONES</td><td $fondoGris>ESTADO CIVIL COINCIDENTE</td><td $fondoGris>CARTA DE VINCULACION Y/O RESOLUCION PROTOCOLIZADA</td><td $fondoGris>No. DE ANOTACION CTL COMPRAVENTA</td><td $fondoGris>SE CANCELA HIPOTECA MAYOR EXTENSION (SI LA HUBIERE)</td><td $fondoGris>PATRIMONIO DE FAMILIA REGISTRADO</td><td $fondoGris>PROHIBICION DE TRANSFERENCIA Y DERECHO DE PREFERENCIA REGISTRADOS</td><td $fondoGris>IMPRESION DE CONSULTA FONVIVIENDA (HOGARES VICTIMAS)</td><td $fondoGris>ELABORO</td><td $fondoGris>APROBO</td><td $fondoGris>SE VIABILIZA JURIDICAMENTE</td><td $fondoGris>OBSERVACION</td></tr>");
