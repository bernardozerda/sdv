<?php
    ...
    $arrprocesorestringidos = array(15, 16, 18, 20, 21, 34, 19, 22, 24, 26, 27, 28, 29, 30, 32, 33, 40);
...
//    echo $_POST['nombre'];die();
    ...
    if (empty($arrErrores)) {
		...
		// Si el estado del proceso es mayor a calificacion

           	 /*
		if (in_array($seqEstadoProceso, $arrprocesorestringidos)) {
                $arrErrores[] = "Error Linea " . ( $numLinea + 1 ) . ": El estado del proceso no esta Permitido";
           	 }
		*/
...
/*
                ...
                case $claFormulario->seqTipoEsquema == 1:
                    ...
                    if (in_array($claFormulario->seqEstadoProceso, array(54, 55, 56))) {
                        if ($claFormulario->seqEstadoProceso != 15){ // Si el estado es asignado no actualiza
    ...
    */
                ...
                case $claFormulario->seqTipoEsquema == 5:
					...
					$estadoActual = $claFormulario->seqEstadoProceso;
                    $arrPost['estadoActual'] = $estadoActual;
					...
					//Añadidos Jueves 9 de Octubre de 2014
					$arrPost['bolPrimeraVerificacion'] = $objCasaMano->bolPrimeraVerificacion;
                    $arrPost['bolRevisionJuridica'] = $objCasaMano->bolRevisionJuridica;
                    $arrPost['bolRevisionTecnica'] = $objCasaMano->bolRevisionTecnica;
