...
if (!isset($_POST['cedula'])) {
    ...
    //die();
    $claSmarty->assign("txtFuncion", "buscarCedula('subsidios/buscarMovilizacion');");
    ...
    if ($seqFormulario == 0) {
        $txtPlantilla = "subsidios/movilizacion.tpl";
    }
...
//        echo "1" . $txtPlantilla = "subsidios/movilizacion.tpl";
//    } 
    else {        
        ...
        if ($validarMovilizacion > 0) {
            ...
            $arrBarrio = obtenerDatosTabla("T_FRM_BARRIO", array("seqBarrio", "txtBarrio"), "seqBarrio", "seqLocalidad = " . $claFormulario->seqLocalidad, "txtBarrio");
            $claFormulario->seqEtapa = obtenerCampo("T_FRM_ESTADO_PROCESO", $claFormulario->seqEstadoProceso, "seqEtapa", "seqEstadoProceso");
            $txtPlantilla = "subsidios/actualizacionMovilizacion.tpl";
        }else{
            echo "Lo sentimos este ciudadano no cumple con los parametros establecidos"; 
        }
    ...
    $sqlPostulante = "SELECT seqFormulario, numDocumento, CONCAT(txtNombre1,' ',txtNombre2,' ',txtApellido1,' ',txtApellido2) AS 'Nombre'
...
and seqFormulario = $seqFormulario";
    ...
    $objRes = $aptBd->execute($sqlPostulante);
    $nombrePostulante = $objRes->fields['Nombre'];
    ...
    $claSmarty->assign("nombrePostulante", $nombrePostulante);
